...
```{r exercise0B-check}
grade_result(
  pass_if(~identical(.result, mtcars[, "mpg"]))
)
```
...
```{r exercise3-check}
grade_result(
  pass_if(~identical(.result, mtcars[-c(1:5), ]))
)
```
...
```{r exercise4-check}
grade_result(
  pass_if(~identical(.result, nrow(mtcars)))
)
```
...
```{r exercise5-check}
grade_result(
  pass_if(~identical(.result, ncol(mtcars)))
)
```
...
```{r exercise6-check}
grade_result(
  pass_if(~identical(.result, rownames(mtcars)))
)
```
...
<div class="mytask">
For every variable in `mtcars` that is less than its average, set it to 0.
</div>
...
```{r exercise10-check}
grade_result(
  pass_if(~identical(.result, dplyr::rename(mtcars, weight = wt, horsepower = hp)))
)
```
...
```{r exercise11-check}
grade_result(
  pass_if(~identical(.result, dplyr::relocate(mtcars, mpg, .after = vs)))
)
```
